...
The system SHALL load JWT signing secrets, SMTP credentials, and LLM provider credentials from explicitly injected runtime configuration and MUST NOT generate production-usable fallback values from repository-owned defaults.

#### Scenario: Starting a deployment-oriented environment
- **WHEN** the application starts in staging or production mode
- **THEN** JWT secrets, enabled SMTP credentials, and enabled LLM credentials MUST come from explicit runtime injection and MUST NOT resolve to hard-coded fallback values
