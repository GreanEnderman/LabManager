...
Define the documentation governance contract that separates current implementation truth, TypeScript prototype limits, Python target-stack direction, and migration-window guidance so planning and implementation decisions do not confuse target-state documents with already-landed behavior.
