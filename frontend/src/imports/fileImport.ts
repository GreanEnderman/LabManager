...
const chemicalFieldOptions: ImportFieldOption[] = [
  ...
  { key: 'spec', label: '规格重量 ml（g）/瓶', type: 'text' },
  { key: 'category', label: '物理特性', type: 'text' },
  { key: 'status', label: '使用状态', type: 'text' },
  { key: 'currentQuantity', label: '当前库存', required: true, type: 'number' },
  { key: 'batchNumber', label: '生产批号', type: 'text' },
  { key: 'openedAt', label: '开封日期', type: 'date' },
  { key: 'expiryDate', label: '保质期', type: 'date' },
  ...
  { key: 'remark', label: '图片/备注', type: 'text' },
]
...
const chemicalHeaderAliases: Record<ChemicalImportField, string[]> = {
  ...
  batchNumber: ['batchnumber', 'batch_number', 'lotnumber', 'lot_number', '生产批号', '批号', '生产批次'],
  openedAt: ['openedat', 'opened_at', 'opendate', 'open_date', '开封日期'],
  expiryDate: ['expirydate', 'expiry_date', 'expirationdate', 'expiration_date', '保质期', '有效期', '失效日期', '到期日期'],
  ...
  updatedAt: ['updatedat', 'updated_at', '更新时间', '采购日期', '使用日期'],
...
function buildChemicalRecords(draft: ParsedImportDraft, mapping: Record<string, ImportFieldKey | null>): ChemicalImportRecord[] {
  ...
  return draft.rows.map((row, index) => {
    ...
    return {
      ...
      batchNumber: getMappedValue(row, mapping, 'batchNumber'),
      openedAt: getMappedValue(row, mapping, 'openedAt'),
      expiryDate: getMappedValue(row, mapping, 'expiryDate'),
...
export function buildTemplateCsv(entityType: ImportEntityType) {
  switch (entityType) {
    case 'chemical':
      return [
        '序号,名称,CAS,规格重量 ml（g）/瓶,物理特性,使用状态（开封/未开封）,数量,生产批号,开封日期,保质期,图片/备注',
        '1,氯化钙，二水,10035-04-8,500g,固体,开封,1,20210425,2024-02-29,2024-04-25,首行样例',
      ].join('\n')
    case 'equipment':
      return [
